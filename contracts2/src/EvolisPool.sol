...
/**
 * @title EvolisPool
 * @notice Connects the PLU (Progressive Liquidity Unlock) to the AMM layer.
 *
 * Flow:
 *   Phase 1 — Bonding Curve Fundraise
 *     · Users buy project tokens at a linearly rising price.
 ...
 *     · LP tokens from PancakeSwap flow to EvolisPool (via _lpRecipient()).
 *
 *   Phase 4 — Milestone
...
contract EvolisPool is ERC20, ReentrancyGuard {
     ...
     * @param _bondingInitialPrice  Starting price in wei per 1e18 tokens
     * @param _bondingSlope    Price increase per token sold (wei per 1e18 tokens / 1e18 tokens)
     ...
     * @dev Also calls controller.setEvolisPool(address(this)) so LP tokens flow here.
     */
    function setController(address _controller) external onlyFactory {
        ...
        // Note: TokenFactory (== controller.deployer) must separately call controller.setEvolisPool(address(this))
     ...
     *         50% hardcoded — governance/DAO to be added in future versions.
     */
