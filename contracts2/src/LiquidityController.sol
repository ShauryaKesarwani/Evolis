...
contract LiquidityController {
    ...
    // EvolisPool integration — set once after deployment
    address public evolisPool;
    bool private _evolisPoolSet;
    
    event LiquidityUnlocked(uint256 indexed epoch, uint256 tokenAmount, uint256 bnbAmount, uint256 liquidity);
    event EvolisPoolSet(address indexed pool);
    ...
    /**
     * @notice Link this controller to EvolisPool — callable once by owner/deployer
     * @dev LP tokens from all liquidity operations will be sent to evolisPool instead of owner
     */
    function setEvolisPool(address _pool) external {
        require(msg.sender == owner || msg.sender == deployer, "Only owner or deployer");
        require(!_evolisPoolSet, "EvolisPool already set");
        require(_pool != address(0), "Invalid pool address");
        _evolisPoolSet = true;
        evolisPool = _pool;
        emit EvolisPoolSet(_pool);
    }

    /// @dev Returns the address that should receive LP tokens
    function _lpRecipient() internal view returns (address) {
        return evolisPool != address(0) ? evolisPool : owner;
    }
    ...
    function initialize(uint256 initialTokenAmount) external payable {
        ...
        PANCAKE_ROUTER.addLiquidityETH{value: msg.value}(
            ...
            0,
            0,
            _lpRecipient(), // LP tokens go to evolisPool if set, else owner
            block.timestamp + 300
        );
    }
